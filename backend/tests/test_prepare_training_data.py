...
def _make_complete_order(
    ...
    *,
    ...
    session.add(
        Customer(customer_id=customer_id, customer_state=customer_state)
    )
    session.add(Seller(seller_id=seller_id, seller_state=seller_state))
    session.commit()

    estimated = datetime(2024, 1, 10, tzinfo=UTC)
    actual = datetime(2024, 1, 10, tzinfo=UTC) if on_time else datetime(2024, 1, 15, tzinfo=UTC)
    _make_order(
        session,
        order_code=order_code,
        customer_id=customer_id,
    ...
    session.add(
        OrderItem(
            order_id=order_code, order_item_id=1, product_id="prod-1", seller_id=seller_id
        )
    )
    session.add(
        OrderPayment(
